...
use firmware_esp32::device_state::DeviceState;
use firmware_esp32::ingress::{IngressResult, PacketIngress};
use firmware_esp32::{
    crc32_with_zeroed_checksum, parse_ack_v1, parse_packet, parse_position_update_v1,
    parse_regional_snapshot_v1, Packet, ParseError, CHECKSUM_OFFSET, REGIONAL_PACKET_SIZE,
...
fn accepts_valid_weather_fixture() {
    ...
    let parsed = parse_regional_snapshot_v1(&packet).expect("valid weather fixture should parse");
...
fn rejects_invalid_precip_fixture() {
    ...
    let error = parse_regional_snapshot_v1(&packet).expect_err("invalid precip should fail");
...
fn rejects_unsupported_version() {
    ...
    let error = parse_regional_snapshot_v1(&packet).expect_err("unsupported version should fail");
...
fn end_to_end_fixture_bytes_to_device_state_estimate() {
    ...
    let weather =
        parse_regional_snapshot_v1(&weather_packet).expect("valid weather fixture should parse");
    let position =
        parse_position_update_v1(&position_packet).expect("valid position fixture should parse");
